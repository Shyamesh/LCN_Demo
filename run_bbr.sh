...
SS_LOG="ss_bbr_flow${FLOW_ID}.txt"
(
...
# Start iperf3 flow (client-side) with BBR
iperf3 -c "$SERVER_IP" -p "$PORT" -C bbr -t "$DURATION" -i "$INTERVAL" --json -R > "bbr_flow${FLOW_ID}.json" &

# Wait for background jobs
wait
echo "✅ BBR flow and ss log completed."
