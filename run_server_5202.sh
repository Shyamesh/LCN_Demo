...
# Start iperf3 server in background (only one flow on port 5202)
echo "Starting iperf3 server..."
iperf3 -s -p 5202 > "$OUTPUT_DIR/iperf_5202.log" 2>&1 &
PID1=$!

# Start tcpdump for port 5202 only
echo "Starting tcpdump..."
tcpdump -i any 'port 5202' -w "$OUTPUT_DIR/combined_capture.pcapng" &
TCPDUMP=$!
